...
int main(void)
{
  ...
  osThreadDef(Taskprint, StartTask02, osPriorityAboveNormal, 0, 128);
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	  if (halfBufferReady == 1)
	  {
		 int i;
		 for(i = 0; i < SAMPLE_BUFFER_SIZE / 2; i++)
		 {
		 	printf("%d ", sample_buffer[i]);
		 	//HAL_UART_Transmit(&huart1, (uint8_t*)sample_buffer, strlen(sample_buffer), 100);
		 }
		 printf("\n");
		 halfBufferReady = 0;
	  }
	  if (halfBufferReady == 2)
	  {
		 int i;
		 for(i = SAMPLE_BUFFER_SIZE; i < SAMPLE_BUFFER_SIZE; i++)
		 {
		 	printf("%d ", sample_buffer[i]);
		 	//HAL_UART_Transmit(&huart1, (uint8_t*)sample_buffer, strlen(sample_buffer), 100);
		 }
